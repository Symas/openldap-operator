...
CLUSTER_NAME="ldap-operator-$(date +%s | rhash --simple - | cut -f 1 -d ' ')"
...
k3d image import -c "${CLUSTER_NAME}" ghcr.io/gpu-ninja/ldap-operator:latest-dev

echo 'Installing operator'

ytt --data-value version=latest-dev -f "${ROOT_DIR}/hack/set-version.yaml" -f "${ROOT_DIR}/config" | kapp deploy -y -a ldap-operator -f -
